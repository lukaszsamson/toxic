defmodule ToxicParser.Cursor do
  ...
  @opaque t :: {charlist(), Toxic.Driver.t(), [token()], [token()], pos_integer(), non_neg_integer()}
  ...
  defp fetch_one({rest, driver, front, back, max_peek, size}) do
    case Toxic.Driver.next(rest, driver) do
      {:ok, tok, rest, driver} -> {:ok, {rest, driver, front, [tok | back], max_peek, size + 1}}
      {:eof, driver} -> {:eof, {[], driver, front, back, max_peek, size}}
      {:error, reason, rest, driver} -> {:error, reason, {rest, driver, front, back, max_peek, size}}
    end
  end
