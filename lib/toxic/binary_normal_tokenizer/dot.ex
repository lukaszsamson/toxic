defmodule Toxic.BinaryNormalTokenizer.Dot do
  ...
  def tokenize_dot(t, line, column, dot_info, scope = scope(column: scope_column), lookbehind)
      when is_binary(t) do
    case strip_horizontal_space_bin(t, 0) do
      ...
      {rest, length} ->
        handle_dot(<<?., rest::binary>>, line, column + length, dot_info, scope, lookbehind)
    end
  end

  # Three Token Operators
  defp handle_dot(<<?., t1, t2, t3, rest::binary>>, line, column, dot_info, scope, lookbehind)
  ...
  # Two Token Operators
  defp handle_dot(<<?., t1, t2, rest::binary>>, line, column, dot_info, scope, lookbehind)
  ...
  defp handle_dot(<<?., t, rest::binary>>, line, column, dot_info, scope, lookbehind)
  ...
  defp handle_dot(<<?., ?(, rest::binary>>, line, column, dot_info, scope, _lookbehind) do
    token = {:dot_call_op, dot_info, :.}
    emit_with_eol(token, <<?(, rest::binary>>, line, column, scope)
  end

  defp handle_dot(<<?., h, rest::binary>>, line, column, dot_info, base_scope, _lookbehind)
       when is_quote(h) do
    scope =
      if h == ?' do
