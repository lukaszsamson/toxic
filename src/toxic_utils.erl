...
split_opts(Args) ->
  case toxic_utils:split_last(Args) of
    {OuterCases, OuterOpts} when is_list(OuterOpts) ->
      case toxic_utils:split_last(OuterCases) of
...
relative_to_cwd(Path) ->
  try toxic_config:get(relative_paths) of
...
stack_location(Line, File) ->
  [{file, toxic_utils:characters_to_list(toxic_utils:relative_to_cwd(File))},
   {line, Line}].
