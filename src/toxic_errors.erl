...
print_diagnostic(#{severity := S, message := M, position := P, file := F} = Diagnostic, ReadSnippet) ->
  ...
  MaybeStack =
    case (F /= nil) orelse toxic_config:is_bootstrap() of
      true -> [];
...
file_warn(Meta, E, Module, Desc) when is_list(Meta) ->
  ...
  case toxic_config:is_bootstrap() of
    true -> ok;
    false ->
...
compile_error(Meta, File, Message) when is_list(Message) ->
  {File, Position} = meta_location(Meta, File),
  raise('Elixir.CompileError', toxic_utils:characters_to_binary(Message), [{file, File} | Position]).
...
cut_snippet({InputString, StartLine, StartColumn, Indentation}, Line, Span) ->
  ...
  toxic_utils:characters_to_binary([Indent, Head, IndentedTail]).
  ...
  Stacktrace =
    case E of
      #{function := {Name, Arity}, module := Module} ->
        [{Module, Name, Arity, [{file, toxic_utils:relative_to_cwd(File)} | Position ]}];
      #{module := Module} when Module /= nil ->
        [{Module, '__MODULE__', 0, [{file, toxic_utils:relative_to_cwd(File)} | Position]}];
      #{} ->
        []
    end,
...
file_format({0, _Column}, File) ->
  toxic_utils:relative_to_cwd(File);
file_format({Line, nil}, File) ->
  file_format(Line, File);
file_format({Line, Column}, File) ->
  io_lib:format("~ts:~w:~w", [toxic_utils:relative_to_cwd(File), Line, Column]);
file_format(0, File) ->
  toxic_utils:relative_to_cwd(File);
file_format(Line, File) ->
  io_lib:format("~ts:~w", [toxic_utils:relative_to_cwd(File), Line]).

meta_location(Meta, File) ->
  case toxic_utils:meta_keep(Meta) of
    {F, L} -> {F, [{line, L}]};
